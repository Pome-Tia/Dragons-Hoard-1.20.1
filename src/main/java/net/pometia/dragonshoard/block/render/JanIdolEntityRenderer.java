package net.pometia.dragonshoard.block.render;
...
public class JanIdolEntityRenderer implements BlockEntityRenderer<JanIdolBlockEntity> {
    ...
    public JanIdolEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    ...
    public void render(JanIdolBlockEntity janAltarBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        float g = (float)janAltarBlockEntity.ticks + f;
        if (!janAltarBlockEntity.isActive()) {
            float h = janAltarBlockEntity.getRotation(0.0F);
            ...
            float h = janAltarBlockEntity.getRotation(f) * (180.0F / (float)Math.PI);
            ...
            matrixStack.pop();
            int l = janAltarBlockEntity.ticks / 66 % 3;
            matrixStack.push();
            matrixStack.translate(0.5F, 0.5F, 0.5F);
            if (l == 1) {
            ...
            this.conduitEye
                    .render(
                            matrixStack,
                            (janAltarBlockEntity.isEyeOpen() ? OPEN_EYE_TEXTURE : CLOSED_EYE_TEXTURE).getVertexConsumer(vertexConsumerProvider, RenderLayer::getEntityCutoutNoCull),
                            i,
                            j
                    );
            matrixStack.pop();
